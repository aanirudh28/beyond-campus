...
import { casebookPath, COUNT_FLOOR, type DownloadCounts } from '@/lib/casebooks'

// Real numbers only — anything under the floor is omitted rather than padded.
function DownloadStat({ total, week }: { total: number; week: number }) {
  if (total < COUNT_FLOOR) return null
  return (
    <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.25)', fontWeight: 500 }}>
      ↓ {total.toLocaleString('en-IN')} downloads
      {week >= 5 && <span style={{ color: 'rgba(147,187,255,0.5)' }}> · {week.toLocaleString('en-IN')} this week</span>}
    </div>
  )
}
...
function CasebookCard({ resource, total, week }: { resource: typeof CASEBOOKS[0]; total: number; week: number }) {
  ...
  const handleDownload = async () => {
    ...
    window.open(casebookPath(resource.id), '_blank')
  ...
  return (
    ...
    >
      ...
      {/* Downloads */}
      <DownloadStat total={total} week={week} />
...
function GuestimateFeaturedCard({ total, week }: { total: number; week: number }) {
  const [downloading, setDownloading] = useState(false)
  const [showCapture, setShowCapture] = useState(false)
  ...
  const handleDownload = async () => {
    ...
    window.open(casebookPath(GUESTIMATE.id), '_blank')
    setDownloading(false)
    setShowCapture(true)
  }
...
export default function ConsultingClient({ counts }: { counts: DownloadCounts }) {
  return (
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 24px 100px' }}>
        ...
        <div className="resource-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 18, marginBottom: 56 }}>
          {CASEBOOKS.map(r => (
            <CasebookCard
              key={r.id}
              resource={r}
              total={counts.total[r.id] ?? 0}
              week={counts.week[r.id] ?? 0}
            />
          ))}
        </div>
        ...
        <div style={{ marginBottom: 64 }}>
          <GuestimateFeaturedCard
            total={counts.total[GUESTIMATE.id] ?? 0}
            week={counts.week[GUESTIMATE.id] ?? 0}
          />
        </div>
