...
import { shareResultCard, shareChallengeCard } from './share'
...
interface Reveal {
  ...
  qStats: { attempts: number; correctPct: number } | null
    ...
    return (
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '56px 20px 80px' }}>
        ...
        <div className="apti-pop" style={{ textAlign: 'center' }}>
          ...
          }}>{isDaily ? '🔥' : isChallenge ? '⚡' : kind === 'review' ? '↺' : '◆'}</div>
          <h1 style={{ fontFamily: 'var(--serif)', fontSize: 38, fontWeight: 400, letterSpacing: -1, margin: '10px 0 6px' }}>
            {isDaily
              ...
              : isChallenge
                ? <>Challenge <em style={{ fontStyle: 'italic', color: COLORS.blueSoft }}>closed.</em></>
                : kind === 'review'
          ...
          <p style={{ color: milestone ? COLORS.blueSoft : COLORS.muted, fontSize: milestone ? 16 : 15, marginBottom: 32, fontWeight: milestone ? 600 : 400 }}>
            {milestone ?? (isDaily
              ...
              : isChallenge
                ? 'Same 3 questions for every student today. Your percentile is waiting on the Today page.'
        ...
        <div className="apti-in" style={{ animationDelay: '0.3s', marginTop: 28 }}>
          ...
          {(isDaily || isChallenge) && s && (
            <button
              onClick={() => (isChallenge
                ? shareChallengeCard({ correct: s.correct, total: s.total, totalTimeMs: s.totalTimeMs })
                : shareResultCard(s))}
              className="apti-option"
              style={{
                width: '100%', padding: '15px 22px', marginBottom: 12,
            ...
            >
              {isChallenge ? 'Challenge a friend ↗' : 'Share the streak ↗'}
            </button>
          )}
    ...
    <div style={{ maxWidth: 620, margin: '0 auto', padding: '20px 20px 120px' }}>
      ...
      {phase === 'reveal' && reveal && (
        <div>
          ...
          {/* community stat — you're not alone in this trap / you're ahead */}
          {reveal.qStats && (reveal.correct ? reveal.qStats.correctPct < 50 : reveal.qStats.correctPct < 80) && (
            <p className="apti-in" style={{
              fontSize: 13, color: COLORS.muted, textAlign: 'center', margin: '0 0 12px',
            }}>
              {reveal.correct
                ? <>Only <Mono style={{ color: COLORS.correct }}>{reveal.qStats.correctPct}%</Mono> get this right. You&rsquo;re in that group.</>
                : <><Mono style={{ color: COLORS.stretch }}>{100 - reveal.qStats.correctPct}%</Mono> get this wrong — now you know the trap.</>}
            </p>
          )}
