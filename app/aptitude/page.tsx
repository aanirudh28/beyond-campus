  ...
  return (
    <main style={{ minHeight: '100vh', background: '#0B0B0F', color: 'white', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 24px', maxWidth: 1100, margin: '0 auto' }}>
        ...
        <Link href="/login?next=/practice" style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
          Log in →
        </Link>
      </nav>
      ...
      <section style={{ maxWidth: 860, margin: '0 auto', padding: '56px 24px 40px', textAlign: 'center' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: 'clamp(15px, 2.3vw, 18px)', lineHeight: 1.7, maxWidth: 640, margin: '0 auto 30px' }}>
        ...
        >
          Start free — find your level in 8 questions →
        </Link>
      ...
      <section style={{ maxWidth: 1000, margin: '0 auto', padding: '0 24px 64px' }}>
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: 20, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
          {/* question card */}
          <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '18px 18px 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
              <span style={{ fontSize: 11.5, letterSpacing: 1, textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', fontWeight: 700 }}>Percentages · Q3 of 10</span>
              <span style={{ fontFamily: 'monospace', fontSize: 12.5, color: 'rgba(255,255,255,0.45)' }}>0:41</span>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '18px 18px 16px', flex: 1 }}>
              <span style={{ fontSize: 11.5, letterSpacing: 1, textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', fontWeight: 700 }}>Apti Rating</span>
              <div style={{ display: 'flex', gap: 24, marginTop: 12 }}>
                {[['Quant', '1287', '▲ 12'], ['Logical', '1194', '▲ 8'], ['Verbal', '1226', '▲ 3']].map(([d, r, w]) => (
                  <div key={d}>
                    <span style={{ fontFamily: 'monospace', fontSize: 24, fontWeight: 600, display: 'block' }}>{r}</span>
                    <span style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.4)' }}>{d}</span>
                    <span style={{ display: 'block', fontFamily: 'monospace', fontSize: 12, color: '#34D399', marginTop: 2 }}>{w}</span>
                  </div>
                ))}
              </div>
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', margin: '12px 0 0', lineHeight: 1.5 }}>Moves every session. Up or down. That&apos;s the point.</p>
            </div>
            <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '18px 18px 16px', flex: 1 }}>
              <span style={{ fontSize: 11.5, letterSpacing: 1, textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', fontWeight: 700 }}>Readiness</span>
              {[['Deloitte', 62], ['ICICI Bank', 71], ['HUL', 48]].map(([name, score]) => (
                <div key={name as string} style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 12 }}>
                  ...
                  <span style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 600 }}>{score}</span>
                </div>
              ))}
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', margin: '12px 0 0', lineHeight: 1.5 }}>One honest number per target company.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ maxWidth: 720, margin: '0 auto', padding: '0 24px 90px', textAlign: 'center' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13.5, marginTop: 18 }}>
          ...
          <Link href="/job-tracker" style={{ color: '#93BBFF', textDecoration: 'none' }}>
      ...
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '28px 24px', textAlign: 'center' }}>
        <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 13, margin: 0 }}>
          ...
          <Link href="/job-tracker" style={{ color: 'rgba(255,255,255,0.45)', textDecoration: 'none' }}>Free Job Tracker</Link>
        </p>
      </footer>
    </main>
  )
